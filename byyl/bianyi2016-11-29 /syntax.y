...
//getchar();getchar();
    ...
    | ID LP Args RP //检查函数在调用时是否定义
    {
		...
		if(is_var_define($1)||is_xcvar_define($1))
		{
	...
	else if(is_var_define($1)||is_xcvar_define($1))
	{
	    printf("Error type 11 at line %d：'%s'is not a function\n",yylineno,$1->content);
	    flag_errfind=1;
	}		
	...
	if((!is_var_define($1))&&(!is_array_define($1))&&(!is_xcvar_define($1)))
	{
	    if(flag_errfind==0)
	    {
